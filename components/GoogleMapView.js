...
const GoogleMapView = ({ selectedSchool, setSelectedSchool }) => {
  ...
  const getHeatmapData = useCallback(() => {
    ...
    return schoolData.map(school => ({
      ...
      weight: school.hours / 3 // Adjust weighting for better visualization
    }));
  }, [isLoaded, schoolData]);

  // Use state to store the heatmap data
  const [heatmapData, setHeatmapData] = useState([]);

  // Update heatmap data when map is loaded
  useEffect(() => {
    if (isLoaded && window.google) {
      setHeatmapData(getHeatmapData());
    }
  }, [isLoaded, getHeatmapData]);

  // Debugging useEffect - log when important states change
  useEffect(() => {
    console.log('Map loaded:', isLoaded);
    if (isLoaded) {
      console.log('Google object available:', Boolean(window.google));
      console.log('Visualization library loaded:', Boolean(window.google?.visualization));
  ...
  const onMapLoad = useCallback((map) => {
    ...
    // Initialize heatmap data again after map is loaded
    if (isLoaded && window.google) {
      setHeatmapData(getHeatmapData());
    }
  ...
  }, [fitBounds, isLoaded, getHeatmapData]);
    ...
    <div className="relative">
      ...
      <div className="relative h-64 md:h-96 lg:h-[500px] border border-gray-300 rounded-lg overflow-hidden">
        <GoogleMap
          ...
          onLoad={onMapLoad}
        >
          {/* Heatmap layer for service intensity - only render when data is available */}
          {isLoaded && window.google && heatmapData.length > 0 && (
            <HeatmapLayer
              data={heatmapData}
              options={{
                radius: 30,
