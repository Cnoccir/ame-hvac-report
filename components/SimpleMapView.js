...
const SimpleMapView = ({ selectedSchool, setSelectedSchool }) => {
  ...
  useEffect(() => {
    console.log("NOTE: Tracking prevention errors are normal on localhost due to browser privacy settings.");
      ...
      <div className="relative h-64 md:h-96 lg:h-[500px] border border-gray-300 rounded-lg overflow-hidden">
        ...
        >
          ...
          {schoolData.map(school => {
            // Determine marker size based on hours (higher hours = larger marker)
            const markerScale = 1.4 + (school.hours / 30);
